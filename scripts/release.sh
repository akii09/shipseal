...
# 10. The images Shipseal shows of itself, and the version strings in the docs, must match
#     the release being cut. That includes `uses: akii09/shipseal@<tag>` in every quick start.
echo "Refreshing showcase images and doc versions..."
set +e
node scripts/refresh-showcase.mjs
SHOWCASE=$?
set -e
if [[ "$SHOWCASE" == "2" ]]; then
  red "BLOCKED: showcase files changed"
  echo "  Fix: review the diff, commit and push it, then run pnpm release again."
  exit 1
elif [[ "$SHOWCASE" != "0" ]]; then
  die "could not refresh showcase files" "run node scripts/refresh-showcase.mjs and see what breaks"
fi
